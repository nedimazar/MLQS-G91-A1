...
class VisualizeDataset:
    ...
    def plot_dataset(self, data_table, columns, match="like", display="line"):
        ...
        if len(columns) > 1:
            f, xar = plt.subplots(
                len(columns), sharex=True, sharey=False, figsize=(50, 5)
            )
        else:
        ...
        for i in range(0, len(columns)):
            ...
            for j in range(0, len(relevant_cols)):
                ...
                if display[i] == "points":
                    xar[i].plot(
                        data_table.index[mask],
                        data_table[relevant_cols[j]][mask],
            ...
            xar[i].set_ylim(
                [
                    min(min_values) - 0.1 * (max(max_values) - min(min_values)),
    ...
    def plot_xy(
        ...
        for i, (x_line, y_line) in enumerate(zip(x, y)):
            ...
            plot_method(
                x_line, y_line, line_styles[i]
            ) if line_styles is not None else plt.plot(x_line, y_line)
    ...
    def plot_binary_outliers(self, data_table, col, outlier_col):
        ...
        f, xar = plt.subplots(dpi=300, figsize=(10, 4))
        xfmt = md.DateFormatter("%H:%M")
        ...
        xar.plot(
            data_table.index[data_table[outlier_col]],
    ...
    def plot_imputed_values(self, data_table, names, col, *values):
        ...
        xar[0].plot(
            data_table.index[data_table[col].notnull()],
            data_table[col][data_table[col].notnull()],
            "b+",
            markersize="2",
        )
        xar[0].legend(
    ...
    def plot_clusters_3d(self, data_table, data_cols, cluster_col, label_cols):
        ...
        for i in range(0, len(label_cols)):
            labels.extend(
                [
                    name
                    for name in list(data_table.columns)
                    if label_cols[i] == name[0 : len(label_cols[i])]
                ]
            )
        ...
        for cluster in clusters:
            ...
            for label in labels:
                rows = data_table.loc[
                    (data_table[cluster_col] == cluster) & (data_table[label] > 0)
                ]
    ...
    def plot_numerical_prediction_versus_real(
        self, train_time, train_y, regr_train_y, test_time, test_y, regr_test_y, label
    ):
        self.legends = {}
        ...
        max_y_value = max(
            max(train_y.tolist()),
            max(regr_train_y.tolist()),
            max(test_y.tolist()),
            max(regr_test_y.tolist()),
        )
        min_y_value = min(
            min(train_y.tolist()),
            min(regr_train_y.tolist()),
            min(test_y.tolist()),
        ...
        plt.annotate(
            "",
            xy=(train_time[0], y_coord_labels),
            xycoords="data",
            xytext=(train_time[-1], y_coord_labels),
            textcoords="data",
            arrowprops={"arrowstyle": "<->"},
        )
        plt.annotate(
            "training set",
            xy=(train_time[int(float(len(train_time)) / 2)], y_coord_labels * 1.02),
            color="blue",
            xycoords="data",
            ha="center",
        )
        plt.annotate(
            "",
            xy=(test_time[0], y_coord_labels),
            xycoords="data",
            xytext=(test_time[-1], y_coord_labels),
            textcoords="data",
            arrowprops={"arrowstyle": "<->"},
        )
        plt.annotate(
            "test set",
            xy=(test_time[int(float(len(test_time)) / 2)], y_coord_labels * 1.02),
            color="red",
            xycoords="data",
            ha="center",
        )
    ...
    def plot_numerical_prediction_versus_real_dynsys_mo(
        self, train_time, train_y, test_time, test_y, dynsys_output, individual, label
    ):
        ...
        self.plot_numerical_prediction_versus_real(
            train_time, train_y, regr_train_y, test_time, test_y, regr_test_y, label
        )
    ...
    def plot_performances(
        self, algs, feature_subset_names, scores_over_all_algs, ylim, std_mult, y_name
    ):
        ...
        for i in range(0, len(feature_subset_names)):
            ...
            plt.errorbar(
                ...
                means,
                yerr=std,
                fmt=self.colors[i % len(self.colors)] + "o",
                markersize="3",
            )
        plt.ylabel(y_name)
    ...
    def plot_performances_classification(
        self, algs, feature_subset_names, scores_over_all_algs
    ):
        self.plot_performances(
            algs, feature_subset_names, scores_over_all_algs, [0.70, 1.0], 2, "Accuracy"
        )

    def plot_performances_regression(
        self, algs, feature_subset_names, scores_over_all_algs
    ):
        self.plot_performances(
            algs,
