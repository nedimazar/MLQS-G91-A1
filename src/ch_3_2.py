...
def main():
    ...
    if FLAGS.mode == "chauvenet":
        ...
        for col in outlier_columns:
            ...
            for c_arg in np.arange(0.25, 10.25, 0.25):
                # And try out all different approaches. Note that we have done some optimization
                # of the parameter values for each of the approaches by visual inspection.
                dataset = OutlierDistr.chauvenet(dataset, col, c_arg)
                # DataViz.plot_binary_outliers(dataset, col, col + "_outlier")
                n = dataset[col + "_outlier"].sum()
                c_values.append(c_arg)
                n_outliers.append(n)
        ...
        for col in outlier_columns:
            ...
            for n_arg in range(1, 20):
                p = deepcopy(dataset)
                p = OutlierDistr.mixture_model(p, col, n=n_arg)

                distribution = p[col + "_mixture"]
        ...
        for col in outlier_columns:
            ...
            fmins = []
            outliers = []
            for dmin in np.arange(0.01, 1.3, 0.015):
                for fmin in np.arange(0.01, 1.3, 0.015):
                    p = deepcopy(dataset)
                    p = OutlierDist.simple_distance_based(
                        p, [col], "euclidean", dmin=dmin, fmin=fmin
                    )
                    n_outliers = p["simple_dist_outlier"].sum()
                    dmins.append(dmin)
                    fmins.append(fmin)
                    outliers.append(n_outliers)
            contents.append((dmins, fmins, outliers))

        with open("distance_outliers.pickle", "wb") as outfile:
        ...
        for col in outlier_columns:
            try:
                ...
                DataViz.plot_dataset(
                    dataset, [col, "lof"], ["exact", "exact"], ["line", "points"]
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["LOF", "distance", "mixture", "chauvenet", "final", "impute"],
    )

    parser.add_argument("--C", type=float, default=2, help="Chauvenet: C parameter")
    ...
    parser.add_argument(
        "--dmin", type=float, default=0.10, help="Simple distance based:  dmin is ... "
    )

    parser.add_argument(
        "--fmin", type=float, default=0.99, help="Simple distance based:  fmin is ... "
    )
